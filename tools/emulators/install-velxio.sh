#!/usr/bin/env bash
# Clone / update Velxio under tools/emulators/velxio (DragonSDR sidequest).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
DEST="${VELXIO_DIR:-$ROOT/tools/emulators/velxio}"
...
cat >"$SUITE_BIN" <<'EOF'
#!/usr/bin/env bash
# Launch local Velxio if possible; otherwise print how to start it.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
VELXIO="${VELXIO_DIR:-$ROOT/tools/emulators/velxio}"

if [[ ! -d "$VELXIO" ]]; then
  echo "Velxio not cloned. Run: $ROOT/tools/emulators/install-velxio.sh" >&2
  exit 1
fi
...
echo
echo "Velxio tree ready: $DEST"
echo "  launch helper: $SUITE_BIN"
